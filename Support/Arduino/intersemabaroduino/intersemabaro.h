...
public:
    ...
    int32_t getTemperatureCelsius(void)
    {
        return AcquireAveragedSampleC(NUM_SAMP_FOR_AVG);
    }  
    
protected:
    virtual int32_t AcquireAveragedSampleCm(const uint8_t nSamples) = 0;
    virtual int32_t AcquireAveragedSampleC(const uint8_t nSamples) = 0;
...
private:
    ...
    virtual int32_t AcquireAveragedSampleC(const uint8_t nSamples)
    {
        long tempAccum = 0;
        for(size_t n = nSamples; n; n--) 
        {
        ...
        long tempAvg = tempAccum / nSamples;
        return tempAvg;
    }
...
private:
    ...
    uint32_t ConvertPressureTemperature(uint32_t pressure, uint32_t temperature)
    {
        ...
        const int32_t temp  = (2000 + (dT * coefficients_[5]) / 8388608) ; // 8388608 == 2^23       // actual temperature

        const int64_t OFF   = static_cast<int64_t>(coefficients_[1]) * 131072 + dT * coefficients_[3] / 64; // offset at actual temperature
        const int64_t SENS  = static_cast<int64_t>(coefficients_[0]) * 65536 + dT * coefficients_[2] / 128; // sensitivity at actual temperature
        
        const int32_t press = ((pressure * SENS / 2097152 - OFF) / 32768);      // temperature compensated pressure
